...
class Job:
    ...
    def can_be_preempted(self, current_time, estimated_borrow_time=1000.0):
        ...
        # Can't preempt if the job has no GPUs (already fully paused)
        ...
        if new_speed_factor == 0:
            # Special case: job will be fully paused (losing its last GPU).
            # The entire borrow time is lost progress.
            slowdown_delay = estimated_borrow_time
        else:
